...
const Billing: React.FC = () => {
    ...
    try {
      if (paymentSettings?.enabled && paymentSettings?.activeGateway === 'paystack') {
        ...
        await startPaystackPayment({
          ...
          onCancel: () => toast.info("Payment canceled."),
        });
        ...
        await trackActivity('subscription_page_visit', { plan_id: plan.id, plan_name: plan.name, amount: plan.price });
        window.location.href = data.url;
      } else {
        toast.error("Payment processing is currently disabled.");
      }
    } catch (error: any) {
    ...
    try {
      if (paymentSettings?.enabled && paymentSettings?.activeGateway === 'paystack') {
        ...
        await startPaystackPayment({
          ...
          reference: reference,
          metadata: { user_id: user.id, type: 'credits', credits: selectedPackage.credits },
          onSuccess: (ref: string) => toast.success(`Payment successful! Ref: ${ref}.`),
          onCancel: () => toast.info("Payment canceled."),
        });
  ...
  const paymentReady = paymentSettings?.enabled && ((paymentSettings.activeGateway === 'paystack' && publicKeys?.paystackPublicKey) || (paymentSettings.activeGateway === 'stripe' && publicKeys?.stripePublicKey));
  const getButtonText = (plan: Plan) => {
    if (!currentUserPlan) return 'Subscribe Now';
    if (plan.id === currentUserPlan.id) return 'Current Plan';
    if (plan.rank > currentUserPlan.rank) return `Upgrade with ${paymentSettings?.activeGateway === 'paystack' ? 'Paystack' : 'Stripe'}`;
    return 'Downgrade';
  };
  ...
  return (
    <div className="container mx-auto py-8 px-4 md:px-6 text-white">
      <Tabs defaultValue="plans" className="w-full">
        ...
        <TabsContent value="plans" className="mt-6">
          <div className="glass-surface">
            <CardHeader>
                ...
                <CardDescription className="text-white/70">Choose a plan that fits your needs.</CardDescription>
            </CardHeader>
            <CardContent className="grid grid-cols-1 md:grid-cols-3 gap-8">
              ...
              plans.map((plan) => (
                <div key={plan.id} className="flex flex-col glass-card p-6">
                  ...
                  <CardContent className="flex-grow p-0 mt-4">
                    <ul className="space-y-2 text-sm text-white/80">{plan.features.map(f => <li key={f} className="flex items-start"><CheckCircle className="h-5 w-5 mr-2 text-green-400 flex-shrink-0 mt-0.5" /><span>{f}</span></li>)}</ul>
                  </CardContent>
                  <CardFooter className="p-0 mt-4">
                    <Button
                      className="w-full"
                      onClick={() => { setSelectedPlanId(plan.id); handleSubscriptionChange(plan.id); }}
        ...
        <TabsContent value="credits" className="mt-6 space-y-8">
          <div className="glass-surface">
            <CardHeader>
              <CardTitle className="flex items-center"><Coins className="mr-2 h-5 w-5" />Your Credits</CardTitle>
              <CardDescription className="text-white/70">Use credits to generate songs, create custom tracks, and more</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="flex items-center justify-between">
                ...
                <Badge variant="outline">20 credits = 1 song generation</Badge>
              </div>
            </CardContent>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {creditPackages.map((pkg, index) => (
              <div key={index} className={`relative glass-card p-6 ${pkg.popular ? 'border-white/20' : ''}`}>
                {pkg.popular && <Badge className="absolute -top-3 left-1/2 transform -translate-x-1/2">Most Popular</Badge>}
                <div className="text-center">
                  <h3 className="text-xl font-bold flex items-center justify-center"><Zap className="mr-2 h-5 w-5" />{pkg.credits} Credits</h3>
                ...
                <div className="text-center">
                  ...
                  >
                    {isLoadingPaymentSettings || isLoadingPublicKeys ? 'Loading...' : !paymentReady ? 'Payments Disabled' : `Purchase`}
                  </Button>
                </div>
              </div>
            ))}
          ...
          <div className="glass-surface">
            <CardHeader>
              ...
              <CardDescription className="text-white/70">Purchase any amount of credits (1 USD = 1 Credit)</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="flex items-center space-x-4">
      ...
      <PaymentDialog
        ...
        title={currentUserPlan ? `Upgrade to ${selectedPlanDetails?.name}` : `Subscribe to ${selectedPlanDetails?.name}`}
