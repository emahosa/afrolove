...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log('Converting to WAV with request:', wavRequest)

    const response = await fetch('https://apibox.erweima.ai/api/v1/wav/generate', {
      method: 'POST',
      headers: {
    ...
    console.log('WAV conversion response:', response.status, responseText)
    ...
    console.error('Error in suno-wav-convert:', error)
    return new Response(
      JSON.stringify({ 
