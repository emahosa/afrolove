...
serve(async (req) => {
  ...
  try {
    ...
    const updatePayload: { status: string; processed_at: string; updated_at: string; admin_notes?: string } = {
      status: 'approved',
    ...
    if (updateError || !updatedRequest) {
      console.error('Error updating payout request to approved:', updateError?.message);
